...
/*
 * For use with "snak2" lamp and "pot-boardDesp-snak2"
 * Tod Kurt / todbot.com
...
void modeLamp()
{
    int l0 = potval; //map( potval, 1023,0, 0,1023); // reverse
    ...
    int llbri = map( dl, 0, (1023/NUM_LEDS_STRAND)+1, 0,255 );

    // reverse & scale
    // int l = map(potval, 1023,0, 0,NUM_LEDS_STRAND); // reverse & scale
    int l = map(potval, 0,1023, 0,NUM_LEDS_STRAND); // reverse & scale
